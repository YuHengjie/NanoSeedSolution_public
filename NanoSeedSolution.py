...
dtypes = [
    ...
    ("Hydrodynamic diameter (nm)", "float32"), ("Concentration (mg/L)", "float32"),
    ("Solution pH", "float32"), ("Solution ionic strength (M)", "float32"), 
...
importance_del = [feature_this_plot.index('Concentration (mg/L)'), 
                feature_this_plot.index('Solution ionic strength (M)')]
...
X_cv_2 = X_cv.loc[:,["Concentration (mg/L)","Solution ionic strength (M)"]]
X_test_2 = X_test.loc[:,["Concentration (mg/L)","Solution ionic strength (M)"]]
...
dot_data = tree.export_graphviz(DT_GS_two, out_file=None,feature_names=['Conc.','Solution IS']) 
...
dot_data = tree.export_graphviz(DT_GS_two_8_Nodes, 
                        out_file=None,
                        feature_names=['Conc.','Solution IS'],) 
graph = graphviz.Source(dot_data) 
...
img = ax.scatter(X['Concentration (mg/L)'], Y, c=X['Solution ionic strength (M)'], 
                linewidths=0, cmap='rainbow')

cb = figure.colorbar(img)
cb.set_label('Solution ionic strength (M) (mol/L)')

ax.set_xlabel('Concentration (mg/L) (mg/L)')
...
ax = figure.add_subplot(111)
sns.scatterplot(
    data=data_label, x="Concentration (mg/L)", y="Zn content (mg/Kg)", hue="Solution ionic strength (M)", 
...
ax.legend(loc="lower right", title='Solution ionic strength (M)')
...
# %% SHAP_Effect_IS
feature = 'Solution ionic strength (M)'
...
rcParams['font.family'] = 'sans-serif'
rcParams['font.sans-serif'] = ['Arial']
feature1 = 'Concentration (mg/L)'
feature2 = 'Solution ionic strength (M)'
shap.dependence_plot((feature1, feature2),shap_interaction_values,X_cv,ax=ax,show=False,
                    display_features=X_cv)


fig.savefig("./Image/SHAP_Interact_MONP_Clay.jpg",dpi=600,bbox_inches='tight')
...
inter1 = pdp.pdp_interact(model=RF_model,
                          dataset=X_cv,
                          model_features=X_cv.columns,
                          features=['Concentration (mg/L)', 'Solution ionic strength (M)'],
                          num_grid_points=[20, 10],
                          percentile_ranges=[(5, 95), (5, 95)])
fig, axes = pdp.pdp_interact_plot(pdp_interact_out=inter1,
                                  feature_names=['Concentration (mg/L)', 'Solution ionic strength (M)'],
                                  plot_type='contour',
                                  x_quantile=True,
                                  plot_pdp=False,figsize=(4.4, 5.5))   
...
X_cv_low = X_cv_2.loc[X_cv_2['Concentration (mg/L)'] <= 50]
Y_cv_low = Y_cv[X_cv_low.index]

X_test_low = X_test_2.loc[X_test_2['Concentration (mg/L)'] <= 50]
...
# %% 浓度大于 50 mg/L 的线性回归
X_cv_high = X_cv_2.loc[X_cv_2['Concentration (mg/L)'] > 50]
Y_cv_high = Y_cv[X_cv_high.index]

X_test_high = X_test_2.loc[X_test_2['Concentration (mg/L)'] > 50]
